...
| **Detect manual scroll & preserve viewport** | ✅ **NEW** | `components/TerminalView.tsx` (scroll handling) | Auto-scroll pauses when user leaves prompt |
| **Keep browser scroll independent of tmux** | ✅ **NEW** | `components/TerminalView.tsx` | Scroll anchoring handled in xterm overlay only |
...
| **Fit to container after mount** | ✅ **DONE** | `hooks/useTerminal.ts` (initialization flow) | Manual dimension calc + `terminal.resize()` |
| **Fit on resize** | ✅ **DONE** | `hooks/useTerminal.ts` (ResizeObserver) | ResizeObserver recalculates + refresh |
...
| Our Implementation | Status | Location | Notes |
|---|---|---|---|
| **Capture visible pane on connect** | ✅ **DONE** | `server.mjs` (`handleSession` setup) | `tmux capture-pane -e` with CRLF normalization |
| **Send to new clients** | ✅ **DONE** | `server.mjs` (`flushHistoryToClient`) | Buffers PTY data until history flush |
...
|---|---|---|---|
| `NEXT_PUBLIC_TERMINAL_DEBUG` flag | ✅ **AVAILABLE** | `hooks/useTerminal.ts`, `components/TerminalView.tsx` | Enables verbose console logging for resize/selection diagnostics |
...
3. **TMUX MOUSE MODE** (User configuration)
   - **Status**: ✅ **ADDRESSED VIA SCRIPT**
   - **Implementation**: `scripts/setup-tmux.sh` now enables `aggressive-resize`, disables drag copy, and keeps mouse scroll enabled
   - **Action**: Re-run the script or mirror the snippet in `~/.tmux.conf`
